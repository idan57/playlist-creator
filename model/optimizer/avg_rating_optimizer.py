...
class AvgRatingOptimizer(object):
    def __init__(self, songs):
        ...
        self.avg_popularity = self.m.sum(
            [(song_para * s_var) / max(1, sum([song.VALUE.value for song in self.songs_vars]))
    ...
    def add_genres_constraint(self, genres):
        for genre in genres:
            sum_genre = self.m.sum(
                [s_var if genre in song.Genres else 0 for s_var, song in zip(self.songs_vars, self.songs)])
            self.m.Equation([sum_genre >= 1])

    def add_artists_constraint(self, artists):
        for artist in artists:
            sum_artist = self.m.sum(
                [s_var if artist in song.Artists else 0 for s_var, song in zip(self.songs_vars, self.songs)])
